...
static uint32_t PWR_GPIO_PIN = 0;

void set_bg96_gpio_instance(uint32_t POWER_PIN, uint32_t PWR_PIN) {

	POWER_GPIO_PIN = POWER_PIN;
	PWR_GPIO_PIN = PWR_PIN;
...
uint8_t set_bg96_power_on(uint8_t retry, uint16_t wait) {
	...
	nrf_gpio_pin_set(PWR_GPIO_PIN);

	wait_100_ms_timer(10);

	nrf_gpio_pin_clear(PWR_GPIO_PIN);
	...
	for (uint8_t i = 0; i < retry; i++) {
		...
		if (bg96_data_write((uint8_t*) BG96_AT_ECHO, strlen(BG96_AT_ECHO),
			...
			if (bg96_evt_state == BG96_OK) {
				...
				NRF_LOG_INFO("BG96 echo success");
				NRF_LOG_FLUSH();
...
void set_bg96_power_off(uint8_t retry, uint16_t wait) {
	...
	NRF_LOG_INFO("BG96 power off start");
	NRF_LOG_FLUSH();
...
uint8_t set_bg96_echo_off(uint16_t wait) {
	...
	NRF_LOG_INFO("BG96 echo off start");
	NRF_LOG_FLUSH();
	...
	if (bg96_data_write((uint8_t*) BG96_AT_ECHO_OFF, strlen(BG96_AT_ECHO_OFF),
		...
		if (bg96_evt_state == BG96_OK) {
			...
			NRF_LOG_INFO("BG96 echo off success");
			NRF_LOG_FLUSH();
	...
	NRF_LOG_INFO("BG96 echo off error");
	NRF_LOG_FLUSH();
...
uint8_t get_bg96_sim_state(uint8_t retry, uint16_t wait) {
	...
	NRF_LOG_INFO("BG96 get sim state start");
	NRF_LOG_FLUSH();
		...
		if (bg96_data_write((uint8_t*) BG96_AT_SIM_READY_READ,
			...
			if (bg96_evt_state == BG96_OK) {
				...
				if (strncmp("READY", (char*) bg96_sim_ready_state, 5) == 0) {
					...
					NRF_LOG_INFO("BG96 get sim state success");
					NRF_LOG_FLUSH();
	...
	NRF_LOG_INFO("BG96 get sim state error");
	NRF_LOG_FLUSH();
...
uint8_t get_bg96_sim_id(uint16_t wait) {
	...
	NRF_LOG_INFO("BG96 get sim id start");
	NRF_LOG_FLUSH();
	...
	if (bg96_data_write((uint8_t*) BG96_AT_SIM_ICCID_READ,
		...
		if (bg96_evt_state == BG96_OK) {
			...
			NRF_LOG_INFO("BG96 get sim id %s", bg96_sim_serial_number);
			NRF_LOG_FLUSH();
	...
	NRF_LOG_INFO("BG96 get sim id error");
	NRF_LOG_FLUSH();
...
uint8_t get_bg96_csq_network_state(uint16_t retry, uint16_t wait) {
	...
	NRF_LOG_INFO("BG96 get csq start");
	NRF_LOG_FLUSH();
	...
	for (uint16_t i = 0; i < retry; i++) {
		...
		if (bg96_data_write((uint8_t*) BG96_AT_CSQ_READ,
			...
			if (bg96_evt_state == BG96_OK) {
				...
				NRF_LOG_INFO("BG96 get csq %d %d", bg96_signal_power,
						bg96_signal_qual);
				NRF_LOG_FLUSH();
	...
	NRF_LOG_INFO("BG96 get csq error");
	NRF_LOG_FLUSH();
...
uint8_t get_bg96_2g_3g_4g_network_registration(uint16_t retry, uint16_t wait) {
	...
	NRF_LOG_INFO("BG96 get 2g/3g/4g network registration start");
	NRF_LOG_FLUSH();
	...
	for (uint16_t i = 0; i < retry; i++) {
		...
		if (bg96_data_write((uint8_t*) BG96_AT_2G_3G_NETWORK_STATE_READ,
			...
			if (bg96_evt_state == BG96_OK) {
				...
				NRF_LOG_INFO("BG96 get 2g/3g network registration %d",
						bg96_2g_3g_4g_network_state);
				NRF_LOG_FLUSH();
		...
		if (bg96_data_write((uint8_t*) BG96_AT_4G_NETWORK_STATE_READ,
			...
			if (bg96_evt_state == BG96_OK) {
				...
				NRF_LOG_INFO("BG96 get 4g network registration %d",
						bg96_2g_3g_4g_network_state);
				NRF_LOG_FLUSH();
	...
	NRF_LOG_INFO("BG96 get 2g/3g/4g network registration error");
	NRF_LOG_FLUSH();
...
uint8_t set_bg96_tcp_ip_pdp_config(uint8_t context_id, uint8_t ip_protocol,
	...
	bg96_write_command_size = sprintf(bg96_write_command,
			"AT+QICSGP=%d,%d,%s,\"\",\"\",0", context_id, ip_protocol,
			apn_name);
...
uint8_t set_bg96_tcp_ip_pdp_activate(uint16_t wait) {
	...
	NRF_LOG_INFO("BG96 set pip activate start");
	NRF_LOG_FLUSH();
	...
	if (bg96_data_write((uint8_t*) BG96_AT_PDP_ACTIVATE,
		...
		if (bg96_evt_state == BG96_OK) {
			...
			NRF_LOG_INFO("BG96 set pip activate success");
			NRF_LOG_FLUSH();
	...
	NRF_LOG_INFO("BG96 set pip activate error");
	NRF_LOG_FLUSH();
...
uint8_t set_bg96_socket_connect(uint8_t socket_number, char *ip, uint16_t port,
	...
	NRF_LOG_INFO("BG96 set socket connect start");
	NRF_LOG_FLUSH();
	...
	bg96_write_command_size = sprintf(bg96_write_command,
			"AT+QIOPEN=1,%d,\"TCP\",\"%s\",%d,0,1", socket_number, ip, port);
	...
	if (bg96_data_write((uint8_t*) bg96_write_command, bg96_write_command_size,
		...
		if (bg96_evt_state == BG96_OK) {
			...
			if (bg96_tcp_socket_connection_state == 0) {
				...
				NRF_LOG_INFO("BG96 set socket connect success");
				NRF_LOG_FLUSH();
	...
	NRF_LOG_INFO("BG96 set socket connect error");
	NRF_LOG_FLUSH();
...
uint8_t set_bg96_socket_write_data_size(uint16_t bg96_tcp_data_size,
	...
	NRF_LOG_INFO("BG96 set socket write data size start");
	NRF_LOG_FLUSH();
	...
	bg96_write_command_size = sprintf(bg96_write_command, "AT+QISEND=%d,%d",
			bg96_tcp_socket_number, bg96_tcp_data_size);
	...
	if (bg96_data_write((uint8_t*) bg96_write_command, bg96_write_command_size,
		...
		if (bg96_evt_state == BG96_OK) {
			...
			NRF_LOG_INFO("BG96 set socket write data size success");
			NRF_LOG_FLUSH();
	...
	NRF_LOG_INFO("BG96 set socket write data size error");
	NRF_LOG_FLUSH();
...
uint8_t set_bg96_socket_write_data(uint8_t *bg96_tcp_data,
	...
	NRF_LOG_INFO("BG96 set socket write data start");
	NRF_LOG_FLUSH();
	...
	if (bg96_tcp_buffer_write((uint8_t*) bg96_write_command,
		...
		if (bg96_evt_state != BG96_OK) {
			...
			NRF_LOG_INFO("BG96 set socket write data error");
			NRF_LOG_FLUSH();
	...
	NRF_LOG_INFO("BG96 set socket write data success");
	NRF_LOG_FLUSH();
